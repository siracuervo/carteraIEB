...
// título público) o por los tickers cortos más comunes (Bonares AL/GD/AE/AO + año,
// Discount DICA/DICY/DICP, Par PARA/PARY). Todos cotizan "cada 100 de nominal".
const BONO_SOBERANO_RE = /^BONOS? |DISCOUNT|DESCUENTO|T[EÍ]TULO P[UÚ]BLICO|BONCER|BOPREAL/;
const TICKER_BONO_SOBERANO_RE = /^(AL|GD|AE|AO)\d|^(DICA|DICY|DICP|DIP0|PARA|PARY|PR13|PR15)$|^B[AB]37D$/;
...
export function clasificar({ activo, ticker, operacion, seccion }) {
  ...
  if (INFO_POR_TICKER[tick]?.sector) {
    return { claseActivo: CLASES.ACCION_LOCAL, sector: INFO_POR_TICKER[tick].sector };
  }
