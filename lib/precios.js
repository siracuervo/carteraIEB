...
async function obtenerPrecioYahoo(symbol) {
  ...
  try {
    ...
    if (meta?.regularMarketPrice) {
      // `precio` = punta vendedora más baja (ask) en rueda, si no el último;
      // `ultimo` = último operado (cierre fuera de rueda).
      resultado = {
        precio: meta.ask > 0 ? meta.ask : meta.regularMarketPrice,
        bid: meta.bid > 0 ? meta.bid : null,
 ...
 * Se valúa a la punta vendedora más baja (ask) cuando hay: en rueda el ask
 * es el precio vivo y fuera de rueda se muestra el cierre (último operado).
      ...
      for (const lista of listas) {
        for (const b of lista || []) {
          ...
          porSimbolo.set(simbolo, {
            // `precio` = punta vendedora más baja (ask) si hay, si no el último;
            // `bid` = punta compradora más alta (para renta fija se valúa al bid);
            // `ultimo` = último operado (cierre fuera de rueda); `q_op` =
            // operaciones de hoy.
            precio: b.px_ask > 0 ? b.px_ask : b.c,
            bid: b.px_bid > 0 ? b.px_bid : null,
            ultimo: b.c,
            q_op: b.q_op ?? null,
            moneda: "ARS",
 ...
 * Es un scrape lento (~4 s): caché propio de 5 min (las fuentes rápidas siguen
 * en 15 s) y timeout corto para no trabar el refresh.
 */
const TTL_RAVA_MS = 300_000;
async function obtenerPrecioRava(ticker) {
  const key = `rava:${String(ticker).toUpperCase()}`;
  const cacheado = cachePrecios.get(key);
  if (cacheado && Date.now() - cacheado.ts < TTL_RAVA_MS) return cacheado.data;
...
/** Velocidades cambiarias en pocos requests: ccl (criptoya y dolarapi en paralelo, fallback Dolarito) y oficial Balanz (ComparaDólar). */
export async function obtenerTipoCambioDolares() {
  ...
  // CCL: criptoya (AL30 24hs, implícito de mercado) y dolarapi
  // (contadoconliqui venta) en PARALELO — gana el primero válido; antes iban en
  // serie y el peor caso sumaba ambas latencias. Dolarito (scrape lento, se
  // actualiza 1 vez por día) solo si fallan las dos.
  const [ccl, oficial] = await Promise.all([
    (async () => {
      const [pxCripto, listaDolarapi] = await Promise.all([
        fetchJSON("https://criptoya.com/api/dolar")
          .then((cripto) => cripto?.ccl?.al30?.["24hs"]?.price ?? null)
          .catch(() => null),
        fetchJSON("https://dolarapi.com/v1/dolares").catch(() => null),
      ]);
      if (pxCripto != null && pxCripto > 0) return pxCripto;
      const porCasa = Object.fromEntries((listaDolarapi || []).map((d) => [d.casa, d]));
      if (porCasa.contadoconliqui?.venta != null) return porCasa.contadoconliqui.venta;
      const desdeDolarito = await obtenerCCLDolarito();
      if (desdeDolarito != null) return desdeDolarito;
      return porCasa.mep?.venta ?? porCasa.oficial?.venta ?? null;
    })(),
    (async () => {
