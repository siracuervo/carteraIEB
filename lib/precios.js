...
export async function obtenerTipoCambioDolares() {
  ...
  // CCL (dolarapi → criptoya → Dolarito) y oficial (Balanz) en paralelo.
  const [ccl, oficial] = await Promise.all([
    (async () => {
      // CCL: criptoya (AL30 24hs, implícito de mercado) primero, después dolarapi
      // (contadoconliqui venta) y último Dolarito (su FAQ se actualiza 1 vez por día).
      try {
        const cripto = await fetchJSON("https://criptoya.com/api/dolar");
        const px = cripto?.ccl?.al30?.["24hs"]?.price;
        if (px != null && px > 0) return px;
      } catch {
        // se prueba la fuente siguiente
      }
      let mepFallback = null;
      let oficialFallback = null;
      try {
        ...
        mepFallback = porCasa.mep?.venta ?? null;
        oficialFallback = porCasa.oficial?.venta ?? null;
      } catch {
        // se prueba la fuente siguiente
      }
      const desdeDolarito = await obtenerCCLDolarito();
      if (desdeDolarito != null) return desdeDolarito;
      return mepFallback ?? oficialFallback ?? null;
    })(),
    (async () => {
