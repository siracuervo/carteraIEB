...
import { importeConDerechos } from "./calculos.js";
...
export async function parseOperacionesDelDia(buffer) {
  ...
  ws.eachRow((row) => {
    ...
    transacciones.push({
      ...
      // Si falta, se estima con derechos de mercado incluidos.
      importeARS: aNumero(row.getCell(8)) ?? Math.sign(cantidad) * importeConDerechos(Math.abs(cantidad * precio), !operacion.startsWith("VENTA")),
      importeDivisas: null,
