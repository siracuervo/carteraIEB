...
export const obtenerDatosCartera = cache(async function obtenerDatosCartera(diaSeleccionado = null, diaTenenciaSeleccionado = null) {
  ...
  if (dia) {
    ...
    // Variación del día de lo que ya se tenía y no se operó ese día (precio de
    // ayer → hoy). Se muestra unificada por ticker junto a lo operado: cada
    // ticker held suma su resultado (ventas + pendiente + variación no operada).
    // Las cantidades compradas ese día quedan afuera (su resultado ya se cuenta
    // en "a mercado"). La renta fija se lista pero no computa en el mosaico.
    ...
    for (const t of posicionesDia) {
      ...
      const prev = precioNumerico(preciosCierre, t.ticker, t.clave);
      const cur = t.precioActual;
      if (prev == null || prev <= 0 || cur == null) continue;
