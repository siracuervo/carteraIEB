...
import { leerTransacciones, leerClasificaciones, leerPortafolioHistorial, leerCierresDiarios, leerCierresManuales, mergeCierresDiarios, leerMovimientosFondos, leerPatrimonioAuto, guardarPatrimonioAuto, leerTraspasosEfectivo } from "./storage.js";
import {
  calcularTenencias,
...
export const obtenerDatosCartera = cache(async function obtenerDatosCartera(diaSeleccionado = null, diaTenenciaSeleccionado = null) {
  ...
  // si no hay cotización ni precioManual, usar el último cierre manual
  {
    ...
    // Un cierre guardado a mano hoy (cierresManuales.json, ej. SPCX) pisa el
    // vivo aunque haya cotización: es el precio que fijó el usuario.
    if (cierresManuales[hoyISO]) {
      tenencias = tenencias.map((t) => {
        if (!t.ticker) return t;
        const pHoy = cierresManuales[hoyISO][t.ticker];
