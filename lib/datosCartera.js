...
import { aperturaDesdePortafolio, lotesPorSleeve, efectivoPorSleeve, dividirTenenciasPorSleeve, serieValorSleeve, serieEfectivoSleeve, balanceCaucion, SLEEVES } from "./sleeves.js";
...
function mapaPreciosDia(fecha, portafolioHistorial, cierres, preciosDisponibles, fechaSesionViva) {
  ...
  for (const [tk, cot] of preciosDisponibles) {
    ...
    mapa.set(tk, cot.ultimo ?? cot.precio);
  }
  return mapa;
}
...
export const obtenerDatosCartera = cache(async function obtenerDatosCartera(diaSeleccionado = null, diaTenenciaSeleccionado = null) {
  ...
  const tenenciasSleeve = dividirTenenciasPorSleeve(tenencias, mapaLotesSleeves, efectivoSleeves).tenencias;
  ...
  const resumen = calcularResumen(tenenciasSleeve);
  ...
  for (const [tk, cot] of preciosDisponibles) {
    if (cot?.precio) preciosHoySleeves.set(tk, cot.ultimo ?? cot.precio);
  }
