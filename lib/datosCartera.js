...
import { leerTransacciones, leerClasificaciones, leerPortafolioHistorial, leerCierresDiarios, mergeCierresDiarios, leerMovimientosFondos, leerPatrimonioAuto, guardarPatrimonioAuto, leerTraspasosEfectivo } from "./storage.js";
import {
  calcularTenencias,
...
export const obtenerDatosCartera = cache(async function obtenerDatosCartera(diaSeleccionado = null, diaTenenciaSeleccionado = null) {
  const [transacciones, overrides, portafolioHistorial, movimientosFondos, patrimonioAuto, traspasosEfectivo] = await Promise.all([
    leerTransacciones(),
    leerClasificaciones(),
  ...
  let fechaSesionMercado = null;
  if (process.env.NEXT_PHASE !== "phase-production-build") {
    const detectado = detectarCierres(preciosDisponibles);
    fechaSesionMercado = detectado.fechaSesion;
    if (detectado.cierres && Object.keys(detectado.cierres).length) {
      // No pisar cierres manuales ya guardados (ej. SPCX hoy 4920): solo faltantes
      const filtrados = {};
    ...
    for (const [tk, cot] of preciosDisponibles) {
      ...
      if (cierres[fechaVivos]?.[tk] != null) continue;
      const px = cot.ultimo ?? cot.precio;
      if (px == null || !(px > 0)) continue;
      vivosHoy[tk] = px;
    }
    if (Object.keys(vivosHoy).length) {
  ...
  // si no hay cotización ni precioManual, usar el último cierre manual
  {
    ...
    // Si hoy guardaste un cierre manual (ej. SPCX 4920), que pise el vivo aunque haya cotización
    if (cierres[hoyISO]) {
      tenencias = tenencias.map((t) => {
        if (!t.ticker) return t;
        const pHoy = cierres[hoyISO][t.ticker];
        if (pHoy == null || pHoy <= 0) return t;
  ...
  // Selección de día: hoy siempre está disponible (aunque no se haya operado)
  // para ver el resultado en vivo de la rueda; por defecto se muestra hoy.
  const fechasOperadas = Array.from(new Set([...transaccionesResueltas.map((t) => t.fecha).filter(Boolean), hoyISO])).sort();
  const diaPorDefecto = hoyISO;
  ...
  const snapshots = portafolioHistorial.map((h) => ({
    fecha: h.fecha,
  ...
  if (process.env.NEXT_PHASE !== "phase-production-build" && preciosEnVivo && ultimoPortafolio && resumen.valorTotalARS > 0) {
    const fechaAuto = fechaSesionMercado || hoyISO;
    const hayPortafolio = portafolioHistorial.some((h) => h.fecha === fechaAuto);
    if (!hayPortafolio) auto = await guardarPatrimonioAuto(fechaAuto, resumen.valorTotalARS);
  }
  if (auto && typeof auto === "object" && Object.keys(auto).length) {
    const fechasPortafolio = new Set(portafolioHistorial.map((h) => h.fecha));
    for (const [fecha, valor] of Object.entries(auto)) {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(fecha) || !(valor > 0) || fechasPortafolio.has(fecha)) continue;
      const idx = snapshots.findIndex((s) => s.fecha === fecha);
      if (idx >= 0) {
        snapshots[idx] = { ...snapshots[idx], fecha, valorTotalARS: valor };
      } else {
        snapshots.push({
          fecha,
          valorTotalARS: valor,
