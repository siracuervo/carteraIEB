...
function preciosCierreAnterior(portafolioHistorial, diaISO) {
  let snapshot = null;
  ...
  const mapa = new Map();
  if (!snapshot) return mapa;
  for (const h of snapshot.tenencias || []) {
...
const precios = await obtenerPrecios(tickers);
  ...
  if (transaccionesResueltas.length) {
    ...
    if (dia) {
      const preciosCierre = preciosCierreAnterior(portafolioHistorial, dia);
      resultadosDia = await calcularResultadosDelDia(transaccionesResueltas, dia, preciosDisponibles, overrides, preciosCierre);
      ...
      if (tenencias.length) {
        ...
        for (const t of tenencias) {
          ...
          // precioAyer = cierre del Portafolio anterior si lo tenemos; si no, se
          // deriva de la variación diaria en vivo (precioHoy / (1 + pct)).
          const cierre = preciosCierre.get(t.ticker) ?? preciosCierre.get(t.clave) ?? null;
          const precioAyer = cierre != null && cierre > 0 ? cierre : t.precioActual / (1 + pct);
          ...
          rendimientosTenencia.push({
            ...
            resultado,
            // Resultado contra el precio de compra original (tenencia total, no del
            // día) para poder mostrar ambos y aclarar cuál es cuál.
            gananciaNoRealizada: t.gananciaNoRealizada ?? null,
            computa: !esRentaFija,
          });
        }
