...
import { TICKERS_NO_MERCADO } from "./clasificacion.js";
...
export const obtenerDatosCartera = cache(async function obtenerDatosCartera() {
  ...
  if (ultimoPortafolio) {
    ...
    const tickers = Array.from(new Set(transacciones.map((t) => t.ticker).filter(Boolean))).filter(
      (t) => !TICKERS_NO_MERCADO.has(t.toUpperCase())
    );
    const precios = await obtenerPrecios(tickers);
    tipoCambioCCL = await obtenerTipoCambioCCL();
    tenencias = calcularTenencias(transacciones, overrides, precios, tipoCambioCCL);
  ...
  const ventasRealizadas = transacciones.length ? await calcularVentasRealizadas(transacciones, overrides) : [];
