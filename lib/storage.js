...
// Caché en memoria de lecturas de Blob con TTL corto (30 s). En Vercel cada
...
// I/O. Se invalida al escribir (escribirJSON) y solo aplica con Blob: en local
...
async function leerJSON(nombre, porDefecto) {
  // Con Blob, lecturas recientes (TTL 30 s) se sirven de memoria.
  if (usaBlob) {
  ...
  if (usaBlob) guardarCache(nombre, resultado);
  return resultado;
}

async function escribirJSON(nombre, valor) {
  // Un write invalida siempre: la próxima lectura vuelve a bajar de Blob...
  if (usaBlob) invalidarCache(nombre);
