...
async function escribirJSON(nombre, valor) {
  ...
  const prefijo = `datos/${nombre}`;
  const creado = await put(`${prefijo}.json`, JSON.stringify(valor, null, 2), {
    access: "public",
...
/**
 * Total del patrimonio visto en vivo por fecha ({ "2026-09-18": 18684085 }).
 ...
 * del día gana, así converge al cierre). Pisa el valor reconstruido en
 * snapshots/serie; ante un Portfolio importado de la misma fecha, el
 * Portfolio (dato oficial IEB) tiene prioridad y este no se aplica.
 */
export async function leerPatrimonioAuto() {
  return leerJSON("patrimonioAuto.json", {});
}

export async function guardarPatrimonioAuto(fecha, valorTotalARS) {
  const actuales = await leerPatrimonioAuto();
  if (Math.abs((actuales[fecha] ?? 0) - valorTotalARS) < 0.5) return actuales;
  actuales[fecha] = valorTotalARS;
