...
// - Con R2_* configurado (Cloudflare R2): ahí. Es la opción actual.
// - Si no, con BLOB_READ_WRITE_TOKEN (Vercel Blob, legado en migración).
...
const usaR2 = hayR2;
const usaBlob = !hayR2 && hayBlob;
const usaRemoto = usaR2 || usaBlob;
...
// Versión de escrituras: se incrementa con cada write (local o Blob). Sirve
...
// URL conocida por archivo (fija desde que se escribe sin sufijo aleatorio).
// Evita el `list` (operación avanzada de Blob, con cupo chico en el plan
// gratuito) en cada lectura: descubrir una sola vez y después bajar directo.
const metaUrls = new Map(); // nombre -> { url, downloadUrl }

/** Localiza el blob de `nombre` (1 sola operación `list`). Prefiere la ruta
 * fija `datos/<nombre>.json`; si todavía no existe (migración), la versión con
 * sufijo más nueva. Guarda el resultado en metaUrls. */
async function descubrirBlob(nombre) {
  ...
  for (const prefijo of candidatos) {
    ...
    const exacto = blobs.find((b) => b.pathname === `${prefijo}.json`);
    const mejor =
      exacto ??
      blobs
    ...
    if (mejor) {
      metaUrls.set(nombre, { url: mejor.url, downloadUrl: mejor.downloadUrl });
      return mejor;
    }
  }
  return null;
...
async function descargarBlob(actual) {
  ...
  if (!res.ok) {
    console.error(`[storage] No se pudo leer ${actual.pathname ?? actual.url}: HTTP ${res.status}`);
    return null;
  }
  try {
...
/**
 * Copia un archivo del Blob legado a R2 (migración por única vez, vía la ruta
 * /api/migrar-r2). Requiere ambos stores configurados. Devuelve { estado }.
 */
export async function migrarArchivoAR2(nombre) {
  if (!hayR2 || !hayBlob) throw new Error("Se necesitan R2_* y BLOB_READ_WRITE_TOKEN configurados");
  const actual = await descubrirBlob(nombre);
  if (!actual) return { estado: "ausente-en-blob" };
  const datos = await descargarBlob(actual);
  if (datos == null) return { estado: "ilegible" };
  await escribirR2(nombre, datos);
  // Verificación: leer de R2 lo recién escrito.
  const verificado = await leerR2(nombre);
  return { estado: verificado != null ? "ok" : "fallo-verificacion" };
}
...
async function leerJSON(nombre, porDefecto) {
  ...
  if (!usaRemoto) {
    ...
    try {
      ...
      if (resultado == null) {
        // Transición Blob→R2: si en R2 no está, se prueba el Blob una vez.
        if (hayBlob) {
          let actual = metaUrls.get(nombre) ?? (await descubrirBlob(nombre));
          let descargado = actual ? await descargarBlob(actual) : null;
          if (descargado == null && actual) {
            metaUrls.delete(nombre);
            actual = await descubrirBlob(nombre);
            descargado = actual ? await descargarBlob(actual) : null;
          }
          if (descargado == null) return porDefecto;
    ...
    try {
      // URL conocida → descarga directa (0 operaciones `list`). Si cambió o
      // venció (404), se redescubre una vez y se reintenta.
      let actual = metaUrls.get(nombre) ?? (await descubrirBlob(nombre));
      let descargado = actual ? await descargarBlob(actual) : null;
      if (descargado == null && actual) {
        metaUrls.delete(nombre);
      ...
      console.error(`[storage] Falló lectura Blob de ${nombre}:`, err.message);
      return porDefecto;
    }
  }
...
async function escribirJSON(nombre, valor) {
  ...
  if (!usaRemoto) {
    try {
      ...
      if (process.env.VERCEL) {
        throw new Error(
          "En Vercel hay que configurar el store remoto (vars R2_* para Cloudflare R2 o BLOB_READ_WRITE_TOKEN para Blob); después redeployá."
        );
      }
      throw err;
    }
  ...
  // a privado si el store lo exige. Ruta FIJA (sin sufijo aleatorio): el put
  // pisa en el lugar (1 sola operación, sin `list` ni borrados posteriores) y
  // la URL queda estable para siempre → las lecturas van directo.
  let creado = null;
  for (const access of ["public", "private"]) {
    try {
      creado = await put(`${prefijo}.json`, cuerpo, { access, contentType: "application/json", allowOverwrite: true, addRandomSuffix: false });
      break;
    } catch (err) {
      if (access === "private" || !/private store|public access/i.test(err.message)) throw err;
    }
  }
  if (creado) metaUrls.set(nombre, { url: creado.url, downloadUrl: creado.downloadUrl ?? creado.url });
}
...
/**
 * Limpieza (una vez por día desde el cron): borra las versiones viejas con
 * sufijo aleatorio de cuando cada put creaba una URL nueva. Con rutas fijas ya
 * no se generan más; esto es solo para podar las acumuladas. Devuelve cuántas
 * borró. Best-effort: nunca tira.
 */
export async function limpiarVersionesViejasBlob() {
  if (!usaBlob) return 0;
  try {
    const { list, del } = await import("@vercel/blob");
    let borradas = 0;
    for (const nombre of ARCHIVOS_RESPALDO.map((a) => a.replace(/\.json$/, ""))) {
      const prefijo = `datos/${nombre}`;
      const { blobs } = await list({ prefix: prefijo, limit: 100 });
      const fija = blobs.find((b) => b.pathname === `${prefijo}.json`);
      // Solo se poda si la ruta fija ya existe (si no, esas son las vigentes).
      if (!fija) continue;
      await Promise.all(
        blobs
          .filter((b) => b.url !== fija.url)
          .map((b) => del(b.url).then(() => { borradas++; }).catch(() => {}))
      );
    }
    return borradas;
  } catch (err) {
    console.error("[storage] limpieza de versiones:", err.message);
    return 0;
  }
}
