...
// Se evalúa en cada acceso (no una vez al importar): en Workers el env puede
// no estar inyectado todavía cuando el módulo se inicializa en frío.
function usaRemoto() {
  return Boolean(
...
// ---------- Cloudflare R2 (API S3-compatible, vía fetch + SigV4) ----------
// A propósito SIN @aws-sdk/client-s3: el SDK toca node:fs al operar y eso
// está bloqueado en el runtime de Workers ("fs.readFile is not implemented").
// Con fetch + WebCrypto funciona igual en local, Vercel y Cloudflare.
...
async function hmacSha256(claveBytes, texto) {
  const key = await crypto.subtle.importKey("raw", claveBytes, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
  const firma = await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(texto));
  return new Uint8Array(firma);
}

function fechaAmz(d = new Date()) {
  return d.toISOString().replace(/[-:]/g, "").slice(0, 15) + "Z"; // YYYYMMDDTHHMMSSZ
}

/** Arma URL + headers firmados (SigV4, región "auto" de R2) para un objeto. */
async function firmarR2(metodo, clave, cuerpo = "") {
  const account = process.env.R2_ACCOUNT_ID;
  const bucket = process.env.R2_BUCKET;
  const host = `${account}.r2.cloudflarestorage.com`;
  const ruta = `/${bucket}/${clave.split("/").map(encodeURIComponent).join("/")}`;
  const amzDate = fechaAmz();
  const fechaCorta = amzDate.slice(0, 8);
  const payloadHash = await sha256Hex(cuerpo);
  const headersFirmados = "host;x-amz-content-sha256;x-amz-date";
  const canonical = [
    metodo,
    ruta,
    "",
  ...
  const scope = `${fechaCorta}/auto/s3/aws4_request`;
  const stringToSign = ["AWS4-HMAC-SHA256", amzDate, scope, await sha256Hex(canonical)].join("\n");
  const kDate = await hmacSha256(new TextEncoder().encode("AWS4" + process.env.R2_SECRET_ACCESS_KEY), fechaCorta);
  const kRegion = await hmacSha256(kDate, "auto");
  const kService = await hmacSha256(kRegion, "s3");
  const kFirma = await hmacSha256(kService, "aws4_request");
  const firma = [...(await hmacSha256(kFirma, stringToSign))].map((b) => b.toString(16).padStart(2, "0")).join("");
  return {
    url: `https://${host}${ruta}`,
    headers: {
      Authorization: `AWS4-HMAC-SHA256 Credential=${process.env.R2_ACCESS_KEY_ID}/${scope}, SignedHeaders=${headersFirmados}, Signature=${firma}`,
      "x-amz-date": amzDate,
      "x-amz-content-sha256": payloadHash,
    },
  };
...
/** Lee `nombre` de R2 (claves fijas, sin listados). null si no existe. */
async function leerR2(nombre) {
  for (const Key of CLAVES_R2(nombre)) {
    const { url, headers } = await firmarR2("GET", Key);
    let res;
    try {
      res = await fetch(url, { headers });
    } catch (err) {
      throw new Error(`R2 GET ${Key}: ${err.message}`);
    }
    if (res.status === 404 || res.status === 400) continue; // NoSuchKey → se prueba la clave siguiente
...
async function escribirR2(nombre, valor) {
  ...
  const { url, headers } = await firmarR2("PUT", `datos/${nombre}`, cuerpo);
  let res;
  try {
    res = await fetch(url, { method: "PUT", headers: { ...headers, "Content-Type": "application/json" }, body: cuerpo });
  } catch (err) {
    throw new Error(`R2 PUT datos/${nombre}: ${err.message}`);
  }
  if (!res.ok) throw new Error(`R2 PUT datos/${nombre}: HTTP ${res.status}`);
}
...
async function leerJSON(nombre, porDefecto) {
  ...
  if (!usaRemoto()) {
    try {
      ...
      // En Workers (Cloudflare) no hay disco: fs tira "not implemented" u
      // otros errores. Ante cualquier fallo de lectura local se devuelve el
      // valor por defecto (igual que archivo ausente) en vez de reventar el
      // render. Si esperabas datos, revisá que las vars R2_* estén cargadas.
      if (err.code !== "ENOENT") {
        console.error(`[storage] No se pudo leer ${nombre} local:`, err.message);
      }
      return porDefecto;
    }
  } else {
...
async function escribirJSON(nombre, valor) {
  ...
  if (!usaRemoto()) {
    try {
      ...
      if (process.env.VERCEL || enWorkers) {
        throw new Error(
          "Sin store remoto: configurá las vars R2_* de Cloudflare R2 en el Worker y redeployá."
        );
      }
      throw err;
    }
