 ...
 * (`precio`: ask si hay, si no el último) — SALVO renta fija, que va a la punta
 * compradora más alta (`bid`: lo que pagan por el bono); fuera de rueda el
 * cierre (último operado, las puntas quedan colgadas). Sin operatoria hoy el
 * ask es una punta vieja y también se usa el último.
 */
export function precioVivo(dato, claseActivo, ahora = new Date()) {
  ...
  if (claseActivo === CLASES.BONO_SOBERANO && dato.bid > 0) return dato.bid;
...
export function actualizarConPreciosVivos(tenencias, precios) {
  return tenencias.map((t) => {
    ...
    // En rueda al ask más bajo (renta fija al bid más alto), fuera de rueda al
