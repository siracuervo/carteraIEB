 ...
 */
export function resolverTickers(transacciones) {
 ...
 * último snapshot, para el selector de días de "Evolución de la cartera". Cada día
 * se compara contra el snapshot inmediatamente anterior disponible (no necesariamente
 * el día de calendario previo, por fines de semana/feriados) — igual criterio que la
 * diaria general. Si un día no tiene Portfolio importado (todavía no llegó, o es un
 * feriado), o es el primer snapshot que existe en todo el historial (no hay contra qué
 * compararlo), queda `disponible: false`.
 */
export function calcularEvolucionSemana(snapshots) {
  if (!snapshots?.length) return [];
  const ordenados = [...snapshots].sort((a, b) => a.fecha.localeCompare(b.fecha));
  const ultimo = ordenados[ordenados.length - 1];
  const lunes = fechaLocal(lunesDeSemana(ultimo.fecha));

  return LETRAS_DIAS_HABILES.map((letra, i) => {
