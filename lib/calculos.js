...
export function actualizarConPreciosVivos(tenencias, precios) {
  return tenencias.map((t) => {
    ...
    // Bonos al último operado (el ask mete ruido en días sin operatoria y
    // rompe la diaria); el resto a la punta vendedora.
    const precioVivo = t.claseActivo === CLASES.BONO_SOBERANO && cotizacion.ultimo != null
      ? cotizacion.ultimo
      : cotizacion.precio;
    const factorPrecio = factorPrecioPorClase(t.claseActivo);
    const valorActual = t.cantidad * precioVivo * factorPrecio;
...
function variacion(desde, hasta) {
  ...
  if (rfDesde != null && rfHasta != null && desde.valorTotalARS != null && hasta.valorTotalARS != null) {
    const dV = desde.valorTotalARS - rfDesde;
    const hV = hasta.valorTotalARS - rfHasta;
    const d = hV - dV;
