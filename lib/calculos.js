...
/**
 * Cierre manual de un ticker para HOY (fecha argentina): fuera de rueda el
 * valor fijado a mano para hoy pisa la cotización viva; en rueda (BYMA
 * 10:30–17:00 ART) manda el vivo y se devuelve null. Los manuales de otras
 * fechas (pasadas o futuras) no aplican: cada uno vale para su propia rueda,
 * para eso tiene fecha. Los días siguientes el valor sigue disponible como
 * último cierre vía cierresDiarios (guardar un manual también lo anota ahí).
...
export function cierreManualVigente(manuales, ticker, ahora = new Date()) {
  ...
  const p = manuales[hoy]?.[ticker] ?? manuales[hoy]?.[tk];
  if (p == null || !(p > 0)) return null;
  return { precio: p, fecha: hoy };
}
