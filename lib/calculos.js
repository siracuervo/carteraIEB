 ...
 * (`precio`: ask si hay, si no el último); fuera de rueda el cierre (último
 * operado, las puntas quedan colgadas). Sin operatoria hoy el ask es una punta
 * vieja y también se usa el último.
 */
...
export function actualizarConPreciosVivos(tenencias, precios) {
  return tenencias.map((t) => {
    ...
    // En rueda al ask más bajo, fuera de rueda al cierre (ver `precioVivo`):
    // el ask fuera de hora mete ruido y rompe la diaria.
