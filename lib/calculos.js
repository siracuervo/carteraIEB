...
/**
 * ¿Opera BYMA ahora? Renta variable lunes a viernes 11:00–17:00 (hora
...
export function mercadoAbierto(ahora = new Date()) {
  ...
  return minutos >= 11 * 60 && minutos < 17 * 60;
}

/**
 * Precio vivo a usar: bonos siempre al último operado; el resto a la punta
 * vendedora solo con mercado abierto y operatoria hoy, si no al último.
 */
export function precioVivo(dato, claseActivo, ahora = new Date()) {
  if (!dato) return null;
  if (claseActivo === CLASES.BONO_SOBERANO) return dato.ultimo ?? dato.precio;
...
export function calcularResumen(tenencias) {
  ...
  const composicion = [
    ...
    { etiqueta: "Renta variable", valorARS: 0 },
    { etiqueta: "Liquidez", valorARS: 0 },
  ];
  ...
  for (const t of tenencias) {
    ...
    const grupo = t.esCash || t.claseActivo === CLASES.EFECTIVO ? 2 : t.claseActivo === CLASES.BONO_SOBERANO ? 0 : 1;
...
export function actualizarConPreciosVivos(tenencias, precios) {
  return tenencias.map((t) => {
    ...
    // Bonos al último operado y resto según rueda (ver `precioVivo`): el ask
    // fuera de hora mete ruido y rompe la diaria.
...
function variacion(desde, hasta) {
  ...
  // Requiere que ambos snapshots traigan `rentaFijaARS` (los reconstruidos sin
  // desglose no la tienen y quedan en null).
  let sinRentaFija = null;
  const rfDesde = desde.rentaFijaARS;
  const rfHasta = hasta.rentaFijaARS;
  if (rfDesde != null && rfHasta != null && desde.valorTotalARS != null && hasta.valorTotalARS != null) {
    // El efectivo apartado a RF (ingresos) tampoco es variable.
    const apDesde = desde.efectivoParaRF ?? 0;
    const apHasta = hasta.efectivoParaRF ?? 0;
...
export function calcularNuevasEnCartera(portafolioHistorial, tenencias) {
  ...
  for (const t of tenencias) {
    ...
    const cantidadAntes = cantidadAnteriorPorTicker.get(t.ticker);
    if (cantidadAntes === undefined) {
      cambiosPorTicker.set(t.ticker, { tipo: "nueva", cantidadAgregada: cantidadAhora, pctAgregado: null });
    } else if (cantidadAhora > cantidadAntes) {
      const cantidadAgregada = cantidadAhora - cantidadAntes;
      cambiosPorTicker.set(t.ticker, {
        tipo: "aumento",
        cantidadAgregada,
