...
function ordenarDiaPorSaldo(ops, saldoInicial) {
  ...
  const candidatos = saldoInicial != null
    ? [saldoInicial]
    : Array.from(new Set(ops.map((t) => t.saldoTenencia - (t.cantidad ?? 0))));
  ...
  if (portafolioHistorial?.length) {
    ...
    for (const clave of clavesSerie) {
      ...
      for (const t of ordenarCronologico(txs)) {
        if (!t.fecha || t.fecha <= fechaBase) continue;
      ...
      if (OPS_COMPRA.includes(op)) {
        ...
        while (porConsumir > 1e-9 && cola.length) {
          ...
          if (ingresoUnit != null && baseUnitaria != null) {
            resultadoDia += (ingresoUnit - baseUnitaria) * usar * factorPrecio;
 ...
 * el importe global de cada boleto) de las compras y ventas posteriores.
 * Devuelve { hubo, tenencias }: hubo es true solo si hubo compras/ventas posteriores.
 */
export function proyectarOperacionesSobrePortafolio(portafolio, transacciones, overrides) {
  ...
  if (!posteriores.length) return { hubo: false, tenencias: null };
  ...
  for (const t of posteriores) {
    const op = (t.operacion || "").toUpperCase().replace(/\s+/g, " ").trim();
    if (t.importeARS != null) deltaEfectivoARS += t.importeARS; // firmado: venta suma, compra resta (ya va con gastos)
