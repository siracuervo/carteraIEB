...
function variacion(desde, hasta) {
  ...
  // Siempre se calcula: si algún snapshot no trae desglose, se asume 0 para no
  // dejar el bloque vacío (el usuario pidió que siempre aparezca).
  let sinRentaFija = null;
  if (desde.valorTotalARS != null && hasta.valorTotalARS != null) {
    const rfDesde = desde.rentaFijaARS ?? 0;
    const rfHasta = hasta.rentaFijaARS ?? 0;
