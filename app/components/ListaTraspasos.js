    ...
    <div className="space-y-3">
      <div className="grid grid-cols-3 gap-2">
        {["trading", "largo", "rentaFija"].map((s) => {
          ...
          return (
            <div key={s} className="rounded-lg border p-2" style={{ borderColor: "var(--border)", background: "var(--surface-1)" }}>
              <div className="text-xs" style={{ color: "var(--text-secondary)" }}>{ETIQUETA[s]}</div>
            ...
            <tbody>
              {ordenados.map((t) => (
                <tr key={t.id} className="border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                  ...
                  <td className="whitespace-nowrap px-3 py-2 text-right tabular-nums font-medium" style={{ color: "var(--text-primary)" }}>
